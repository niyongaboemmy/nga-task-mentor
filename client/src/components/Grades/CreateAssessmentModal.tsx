    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
        ...
        <div className="px-6 py-5 space-y-4">
          {/* Subject selector */}
          <div>
            <label className="block text-sm text-gray-500 mb-1.5">Class</label>
            <select
              value={courseId}
              onChange={(e) => setCourseId(e.target.value ? Number(e.target.value) : "")}
              disabled={isEdit}
              className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 bg-white disabled:bg-gray-50 disabled:text-gray-400"
            >
          ...
          {/* Assessment Type */}
          <div>
            <label className="block text-sm text-gray-500 mb-1.5">Assessment Type</label>
            <select
              value={assessmentType}
              onChange={(e) => setAssessmentType(e.target.value as AssessmentType)}
              className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 bg-white"
            >
          ...
          {/* Assessment Number */}
          <div>
            <label className="block text-sm text-gray-500 mb-1.5">Assessment Number</label>
            <input
              type="number"
              min={1}
          ...
          {/* Date */}
          <div>
            <label className="block text-sm text-gray-500 mb-1.5">Select Date</label>
            <div className="relative">
              <input
                type="date"
                value={assessmentDate}
                onChange={(e) => setAssessmentDate(e.target.value)}
                className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 pr-10"
              />
          ...
          {/* Maximum Marks */}
          <div>
            <label className="block text-sm text-gray-500 mb-1.5">Maximum Marks</label>
            <input
              type="number"
              min={1}
          ...
          <label className="flex items-center gap-3 cursor-pointer">
            ...
            />
            <span className="text-sm text-gray-700">Add to Final Grade</span>
          </label>
        </div>
