import { QuestionType, QuestionDataType, QuestionValidationResult } from '../types/quiz.types';

// Validation functions for each question type
export class QuestionValidator {
  static validateQuestionData(questionType: QuestionType, data: any): QuestionValidationResult {
    const errors: string[] = [];
    ...
    switch (questionType) {
      ...
      case 'dropdown':
        return this.validateDropdown(data);
      case 'numerical':
        return this.validateNumerical(data);
      case 'algorithmic':
        return this.validateAlgorithmic(data);
      case 'short_answer':
  ...
  private static validateSingleChoice(data: any): QuestionValidationResult {
    ...
    if (data.options && data.options.some((opt: string) => typeof opt !== 'string' || opt.trim() === '')) {
      errors.push('All options must be non-empty strings');
    }

    if (typeof data.correct_option_index !== 'number' || data.correct_option_index < 0 || data.correct_option_index >= data.options?.length) {
      errors.push('Correct option index must be a valid index within the options array');
    }
  ...
  private static validateMultipleChoice(data: any): QuestionValidationResult {
    ...
    if (!data.correct_option_indices || !Array.isArray(data.correct_option_indices) || data.correct_option_indices.length === 0) {
    ...
    if (data.correct_option_indices) {
      data.correct_option_indices.forEach((index: number, i: number) => {
        if (typeof index !== 'number' || index < 0 || index >= data.options?.length) {
          errors.push(`Correct option index ${index} at position ${i} is invalid`);
        }
      });
    }
    ...
    if (data.correct_option_indices && data.correct_option_indices.length === 1) {
      warnings.push('Only one correct option selected - consider using single choice instead');
    }
  ...
  private static validateTrueFalse(data: any): QuestionValidationResult {
    ...
    if (typeof data.correct_answer !== 'boolean') {
      errors.push('True/False questions must have a boolean correct_answer field');
    }
  ...
  private static validateMatching(data: any): QuestionValidationResult {
    ...
    if (!data.left_items || !Array.isArray(data.left_items) || data.left_items.length === 0) {
      errors.push('Matching questions must have at least one left item');
    }

    if (!data.right_items || !Array.isArray(data.right_items) || data.right_items.length === 0) {
      errors.push('Matching questions must have at least one right item');
    }

    if (data.left_items && data.right_items && data.left_items.length !== data.right_items.length) {
      warnings.push('Number of left and right items differ - this may confuse students');
    }
  ...
  private static validateFillBlank(data: any): QuestionValidationResult {
    ...
    if (!data.acceptable_answers || !Array.isArray(data.acceptable_answers) || data.acceptable_answers.length === 0) {
      errors.push('Fill blank questions must have at least one acceptable answer');
    }

    const blankCount = (data.text_with_blanks?.match(/\{\{blank\}\}/g) || []).length;
    if (blankCount === 0) {
      errors.push('Text must contain at least one {{blank}} placeholder');
    }

    if (data.acceptable_answers && blankCount > 0 && data.acceptable_answers.length !== blankCount) {
      errors.push(`Number of acceptable answers (${data.acceptable_answers.length}) must match number of blanks (${blankCount})`);
    }
  ...
  private static validateDropdown(data: any): QuestionValidationResult {
    ...
    if (!data.text_with_dropdowns || typeof data.text_with_dropdowns !== 'string') {
      errors.push('Dropdown questions must have text_with_dropdowns field');
    }

    if (!data.dropdown_options || !Array.isArray(data.dropdown_options) || data.dropdown_options.length === 0) {
      errors.push('Dropdown questions must have at least one dropdown option set');
    }

    const dropdownCount = (data.text_with_dropdowns?.match(/\{\{dropdown\}\}/g) || []).length;
    if (dropdownCount === 0) {
      errors.push('Text must contain at least one {{dropdown}} placeholder');
    }

    if (data.dropdown_options && dropdownCount > 0 && data.dropdown_options.length !== dropdownCount) {
      errors.push(`Number of dropdown option sets (${data.dropdown_options.length}) must match number of dropdowns (${dropdownCount})`);
    }

    if (data.dropdown_options) {
      data.dropdown_options.forEach((optionSet: any, index: number) => {
        if (!optionSet.options || !Array.isArray(optionSet.options) || optionSet.options.length < 2) {
          errors.push(`Dropdown option set ${index} must have at least 2 options`);
        }
      });
    }
  ...
  private static validateNumerical(data: any): QuestionValidationResult {
    ...
    if (data.acceptable_range) {
      if (typeof data.acceptable_range.min !== 'number' || typeof data.acceptable_range.max !== 'number') {
  ...
  private static validateAlgorithmic(data: any): QuestionValidationResult {
    ...
    if (!data.algorithm_description || typeof data.algorithm_description !== 'string') {
  ...
  private static validateShortAnswer(data: any): QuestionValidationResult {
    ...
    if (data.max_length && (typeof data.max_length !== 'number' || data.max_length < 1)) {
  ...
  private static validateCoding(data: any): QuestionValidationResult {
    ...
    if (data.time_limit && (typeof data.time_limit !== 'number' || data.time_limit < 1)) {
      errors.push('Time limit must be a positive number');
    }

    if (data.memory_limit && (typeof data.memory_limit !== 'number' || data.memory_limit < 1)) {
  ...
  private static validateLogicalExpression(data: any): QuestionValidationResult {
    const errors: string[] = [];
    const warnings: string[] = [];
    ...
    if (!data.variables || !Array.isArray(data.variables) || data.variables.length === 0) {
      errors.push('Logical expression questions must have at least one variable');
    }

    if (!data.correct_expression || typeof data.correct_expression !== 'string') {
  ...
  private static validateDragDrop(data: any): QuestionValidationResult {
    ...
    if (!data.draggable_items || !Array.isArray(data.draggable_items) || data.draggable_items.length === 0) {
      errors.push('Drag and drop questions must have at least one draggable item');
    }
    ...
    if (data.drop_zones) {
      data.drop_zones.forEach((zone: any, index: number) => {
        if (typeof zone.x !== 'number' || typeof zone.y !== 'number' || typeof zone.width !== 'number' || typeof zone.height !== 'number') {
          errors.push(`Drop zone ${index} must have numeric x, y, width, and height`);
        }
    ...
    if (data.draggable_items && data.drop_zones) {
      const totalCorrectSlots = data.drop_zones.reduce((sum: number, zone: any) => sum + zone.correct_items.length, 0);
      if (totalCorrectSlots > data.draggable_items.length) {
        warnings.push('More correct slots than draggable items - some items may need to be used multiple times');
      }
    }
  ...
  private static validateOrdering(data: any): QuestionValidationResult {
    ...
    if (data.items) {
      ...
      if (minOrder !== 1 || maxOrder !== data.items.length) {
        warnings.push('Order values should form a consecutive sequence starting from 1');
      }
    }
