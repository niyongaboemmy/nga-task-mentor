import { Table, Column, Model, DataType, ForeignKey, BelongsTo, BeforeSave, BeforeUpdate } from 'sequelize-typescript';
...
export type SubmissionCreationAttributes = Omit<ISubmissionAttributes, 'id' | 'created_at' | 'updated_at'>;
...
export class Submission extends Model<ISubmissionAttributes, SubmissionCreationAttributes> {
  @Column({
  ...
  static validateSubmission(submission: ISubmissionAttributes): void {
    ...
    if (submission.file_submissions !== null &&
  ...
  @BeforeSave
  @BeforeUpdate
  protected static async validateBeforeSave(instance: Submission): Promise<void> {
    try {
      ...
      if (instance.isNewRecord || instance.changed('status')) {
        if (instance.status === 'submitted' || instance.status === 'resubmitted') {
          if (!instance.submitted_at) {
            instance.submitted_at = new Date();
          }

          const assignment = instance.assignment || (await Assignment.findByPk(instance.assignment_id));

          if (assignment) {
            instance.is_late = instance.submitted_at > (assignment as any).due_date;
            ...
            if ((submissionType === 'file' || submissionType === 'both') &&
                (!instance.file_submissions || instance.file_submissions.length === 0)) {
